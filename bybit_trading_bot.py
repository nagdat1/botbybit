#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Updated: 2024-10-29 - Fixed InlineKeyboardButton error
"""
...
📧 للدعم: استخدم أمر /start
    ...
    if data == "market_spot":
        ...
        # ✅ إصلاح: استخدام user_manager.update_user_settings لضمان التحديث في الذاكرة وقاعدة البيانات
        if user_id is not None:
    ...
    if data == "market_futures":
        ...
        # ✅ إصلاح: استخدام user_manager.update_user_settings لضمان التحديث في الذاكرة وقاعدة البيانات
        if user_id is not None:
            user_manager.update_user_settings(user_id, {'market_type': 'futures'})
        # إعادة تعيين حالة إدخال المستخدم
        if user_id is not None and user_id in user_input_state:
            del user_input_state[user_id]
        await settings_menu(update, context)
    ...
    if data == "account_real":
        ...
        # ✅ إصلاح: استخدام user_manager.update_user_settings لضمان التحديث في الذاكرة وقاعدة البيانات
        if user_id is not None:
            user_manager.update_user_settings(user_id, {'account_type': 'real'})
        # إعادة تعيين حالة إدخال المستخدم
        if user_id is not None and user_id in user_input_state:
            del user_input_state[user_id]
        await settings_menu(update, context)
    ...
    if data == "account_demo":
        ...
        # ✅ إصلاح: استخدام user_manager.update_user_settings لضمان التحديث في الذاكرة وقاعدة البيانات
        if user_id is not None:
            user_manager.update_user_settings(user_id, {'account_type': 'demo'})
        # إعادة تعيين حالة إدخال المستخدم
        if user_id is not None and user_id in user_input_state:
            del user_input_state[user_id]
        await settings_menu(update, context)
...
✅ **مميزات الرابط الشخصي:**
• يستخدم إعداداتك الخاصة تلقائياً
...
**الخطوة 1:** افتح استراتيجيتك في TradingView
**الخطوة 2:** اذهب إلى `Settings → Notifications`
**الخطوة 3:** أضف `Webhook URL`
**الخطوة 4:** الصق رابطك الشخصي أعلاه
**الخطوة 5:** في حقل `Message`، استخدم هذه الصيغة:
...
💡 **الإشارات المدعومة:**
• `buy` / `long` - فتح صفقة شراء
• `sell` / `short` - فتح صفقة بيع
• `close` - إغلاق كامل
• `partial_close` - إغلاق جزئي (مع `percentage`)

━━━━━━━━━━━━━━━━━━━━━━

⚙️ **الإعدادات الحالية:**
📊 نوع السوق: {user_data.get('market_type', 'spot').upper() if user_data else 'غير محدد'}
🏦 نوع الحساب: {user_data.get('account_type', 'demo').upper() if user_data else 'غير محدد'}
💰 مبلغ التداول: {user_data.get('trade_amount', 100)} USDT
⚡ الرافعة المالية: {user_data.get('leverage', 1)}x
▶️ حالة البوت: {'🟢 نشط' if user_data and user_data.get('is_active') else '🔴 متوقف'}

━━━━━━━━━━━━━━━━━━━━━━

⚠️ **ملاحظات مهمة:**
🔒 لا تشارك هذا الرابط مع أحد
📊 سيتم استخدام إعداداتك الحالية عند تنفيذ الإشارات
⚙️ يمكنك تعديل الإعدادات من القائمة الرئيسية
▶️ تأكد من تشغيل البوت قبل إرسال الإشارات
...
عند التفعيل، أي صفقة تفتحها على حسابك ستُرسل تلقائياً لجميع متابعيك!
            ...
            await update.message.reply_text(
                "⚠️ **تحذير خطير جداً!**\n\n"
                "🔥 **هذا الإجراء سيحذف كل شيء نهائياً:**\n\n"
                "❌ **سيتم حذف:**\n"
                "• ✅ جميع حسابات المستخدمين\n"
                "• ✅ جميع الصفقات والإشارات\n"
                "• ✅ جميع الإعدادات والبيانات\n"
                "• ✅ الذاكرة المؤقتة (Cache)\n"
                "• ✅ سجلات التداول\n"
                "• ✅ ملف قاعدة البيانات بالكامل\n"
                "• ✅ جميع النسخ الاحتياطية\n\n"
                "🔄 **سيتم إعادة إنشاء:**\n"
                "• قاعدة بيانات جديدة فارغة تماماً\n"
                "• الجداول فقط بدون أي بيانات\n\n"
                "👥 **المستخدمون الجدد:**\n"
                "• سيبدأون من الصفر عند /start\n"
                "• رصيد افتراضي: 10000 USDT\n"
                "• إعدادات افتراضية من config.py\n\n"
                "⚠️ **سيبقى فقط:**\n"
                "• حساب المطور (أنت)\n"
                "• البنية الأساسية للبوت\n\n"
...
📱 استخدم /start للمحاولة مرة أخرى
            ...
            try:
                ...
                if amount > 0:
                    ...
                    # ✅ إصلاح: استخدام user_manager.update_user_settings لضمان التحديث في الذاكرة وقاعدة البيانات
                    user_manager.update_user_settings(user_id, {'trade_amount': amount})
                    # إعادة تعيين حالة إدخال المستخدم
                    del user_input_state[user_id]
                    if update.message is not None:
            ...
            try:
                ...
                if 1 <= leverage <= 100:
                    ...
                    # ✅ إصلاح: استخدام user_manager.update_user_settings لضمان التحديث في الذاكرة وقاعدة البيانات
                    user_manager.update_user_settings(user_id, {'leverage': leverage})
                    # إعادة تعيين حالة إدخال المستخدم
                    del user_input_state[user_id]
                    if update.message is not None:
