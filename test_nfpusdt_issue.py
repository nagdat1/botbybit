...
# -*- coding: utf-8 -*-
"""
فحص مشكلة فشل تنفيذ الإشارة مع NFPUSDT
"""
...
async def test_nfpusdt_order():
    """اختبار طلب NFPUSDT"""
    logger.info("اختبار طلب NFPUSDT...")
    
    try:
        # إنشاء حساب Bybit
        account = BybitRealAccount(BYBIT_API_KEY, BYBIT_API_SECRET)
        
        # جلب السعر الحالي لـ NFPUSDT
        ticker = account.get_ticker('spot', 'NFPUSDT')
        if not ticker:
            logger.error("فشل في جلب السعر لـ NFPUSDT")
            return False
        
        current_price = float(ticker['lastPrice'])
        logger.info(f"السعر الحالي لـ NFPUSDT: ${current_price}")
        
        # حساب الكمية بناءً على المبلغ المطلوب (50 دولار)
        amount_usd = 50.0
        qty = amount_usd / current_price
        logger.info(f"المبلغ المطلوب: ${amount_usd}")
        logger.info(f"الكمية المحسوبة: {qty:.6f} NFP")
        ...
        if test_order and test_order.get('success'):
            ...
            logger.error(f"فشل طلب NFPUSDT: {error_code} - {error_msg}")
            return False
        
    except Exception as e:
        logger.error(f"خطأ في اختبار NFPUSDT: {e}")
        return False

async def test_nfpusdt_signal():
    """اختبار إشارة NFPUSDT"""
    logger.info("اختبار إشارة NFPUSDT...")
    ...
    try:
        ...
        if result.get('success'):
            logger.info("نجح تنفيذ إشارة NFPUSDT!")
            return True
        else:
            logger.error(f"فشل تنفيذ إشارة NFPUSDT: {result.get('message', 'Unknown error')}")
            logger.error(f"تفاصيل الخطأ: {result}")
            return False
            
    except Exception as e:
        logger.error(f"خطأ في اختبار إشارة NFPUSDT: {e}")
        import traceback
        traceback.print_exc()
        return False

async def test_different_amounts():
    """اختبار مبالغ مختلفة لـ NFPUSDT"""
    logger.info("اختبار مبالغ مختلفة لـ NFPUSDT...")
    ...
    try:
        ...
        for amount_usd in test_amounts:
            qty = amount_usd / current_price
            logger.info(f"اختبار ${amount_usd} = {qty:.6f} NFP")
...
async def main():
    """الدالة الرئيسية"""
    logger.info("فحص مشكلة فشل تنفيذ الإشارة مع NFPUSDT...")
    
    # اختبار 1: طلب مباشر
    direct_success = await test_nfpusdt_order()
    
    # اختبار 2: إشارة كاملة
    signal_success = await test_nfpusdt_signal()
    
    # اختبار 3: مبالغ مختلفة
    min_amount = await test_different_amounts()
    
    # تقرير النتائج
    logger.info("\n" + "="*50)
    logger.info("نتائج فحص NFPUSDT:")
